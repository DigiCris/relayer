...
/*!
* \brief       Read all that matches calls in relayerRPC
* \details     Search in the database relayerRPC that matches calls and returns them in an array.
* \param       (INT) calls searching table by matching it.
...
*/
function getByCalls($calls) {
    $register = new relayerRPC();
    $success['response'] = $register->readCalls($calls);

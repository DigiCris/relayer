...
/*!
* \brief       Read all that matches endpoint in relayerRPC
* \details     Search in the database relayerRPC that matches endpoint and returns them in an array.
* \param       (VARCHAR) endpoint searching table by matching it.
...
function getByEndpoint($endpoint) {
    ...
    $success['response'] = $register->readEndpoint($endpoint);

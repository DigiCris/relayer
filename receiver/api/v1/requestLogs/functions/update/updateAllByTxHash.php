...
* \details      Update the information of requestLogs table, searching it by txHash.
* \param      $id     (INT)  id to update.  1 unique id for each request
* \param      $request     (VARCHAR)  request to update. request and signature to relay
* \param      $txHash       (VARCHAR)    Identifier of the requestLogs to update. Hash of the transaction
...
* \return      $success['msg'] (string) Returns a message explaining the status of the execution.
    ...
    * \return •    'We could not find the txHash you are trying to update.' -> When the requestLogs's field txHash does not exist or it is not found in the database.
*/

function updateAllByTxHash($id, $request, $txHash, $status, $timestamp, $from, $nonce, $emailSent, $retry) {
    $register = new requestLogs();

    $success['response'] = $register->readTxHash($txHash);
    ...
    if($success['response']['txHash'] == $txHash) {
        ...
        $success['response'] = $register->updateAllByTxHash($txHash);
        $success['response'] = $register->readTxHash($txHash);
        if($success['response']['status'] == $status and $success['response']['txHash'] == $txHash) {

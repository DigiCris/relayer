...
/*!
* \brief       Read all that matches retry in requestLogs
* \details     Search in the database requestLogs that matches retry and returns them in an array.
* \param       (INT) retry searching table by matching it.
...
*/
function getByRetry($retry) {
    $register = new requestLogs();
    $success['response'] = $register->readRetry($retry);

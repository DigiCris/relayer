...
/*!
* \brief       Read all that matches timestamp in requestLogs
* \details     Search in the database requestLogs that matches timestamp and returns them in an array.
* \param       (DATE) timestamp searching table by matching it.
...
function getByTimestamp($timestamp) {
    ...
    $success['response'] = $register->readTimestamp($timestamp);
